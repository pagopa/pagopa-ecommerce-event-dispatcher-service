...
| AUTHORIZATION_OUTCOME_FIRST_ATTEMPT_DELAY_SECONDS                              | Delay before the first NPG get-state attempt for transactions blocked in `AUTHORIZATION_REQUESTED`. If a transaction is blocked in `AUTHORIZATION_COMPLETED` or `CLOSURE_REQUESTED` (for both NPG and REDIRECT gateways), the service retrieves the authorization data and performs a PATCH auth-request to trigger the transaction update | integer |               |
