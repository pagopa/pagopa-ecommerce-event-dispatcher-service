...
| SEND_PAYMENT_RESULT_TIMEOUT_SECONDS                          | Max time (in seconds) to be awaited for the send payment result to be received for a given transaction                                                                                     | integer |         |
| SEND_PAYMENT_RESULT_EXPIRATION_OFFSET_SECONDS                | Send payment result max time to wait expiration offset. This value is used for consider expired also transactions that have a send payment result timeout less than this offset as expired | integer |         |
