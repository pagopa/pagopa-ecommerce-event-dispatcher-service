package it.pagopa.ecommerce.eventdispatcher.streams
...
import it.pagopa.ecommerce.eventdispatcher.streams.commands.EventDispatcherGenericCommand
import it.pagopa.ecommerce.eventdispatcher.streams.commands.EventDispatcherReceiverCommand
...
class RedisStreamConsumer(
    @Autowired
    private val redisStreamReceiver: StreamReceiver<String, ObjectRecord<String, LinkedHashMap<*, *>>>,
    @Autowired
    ...
    @Autowired
    private val inboundChannelAdapterLifecycleHandlerService: InboundChannelAdapterLifecycleHandlerService,
    ...
    /*
    private val REDIS_EVENT_ID = "REDIS_EVENT_ID"
    private val REDIS_EVENT_TIMESTAMP = "REDIS_EVENT_TIMESTAMP"
    private val REDIS_EVENT_STREAM_KEY = "REDIS_EVENT_STREAM_KEY"
    ...
    override fun onApplicationEvent(event: ApplicationReadyEvent) {
        ...
        eventStreamPipelineWithRetry().subscribeOn(Schedulers.parallel()).subscribe { record ->
            runCatching {
                val event = objectMapper.convertValue(record.value, EventDispatcherGenericCommand::class.java)
                processStreamEvent(event)
            }.onFailure { ex ->
                logger.error("Error processing redis stream event", ex)
            }
        }
    }
    ...
    fun hanldeEventReceiverCommand(command: EventDispatcherReceiverCommand) {
        ...
        val isTargetedByCommand = commandTargetVersion == null || currentDeploymentVersion == commandTargetVersion
        logger.info(
        ...
        if (isTargetedByCommand) {
            ...
            logger.info("Current deployment version not targeted by command, command will not be processed")
        }
    }
}
