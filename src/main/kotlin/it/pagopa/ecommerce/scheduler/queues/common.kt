...
fun updateTransactionToRefunded(
    transaction: BaseTransaction,
    transactionsRefundedEventStoreRepository: TransactionsEventStoreRepository<TransactionRefundedData>,
    transactionsViewRepository: TransactionsViewRepository,
    ...
    return Mono.just(tx)
        ...
        .flatMap {
            ...
            when (refundResponse.refundOutcome) {
                "OK" -> updateTransactionToRefunded(transaction, transactionsEventStoreRepository, transactionsViewRepository)
                else ->
...
fun paymentNoticeDocuments(paymentNotices: List<it.pagopa.ecommerce.commons.domain.v1.PaymentNotice>): List<PaymentNotice> {
...
fun isTransactionRefundable(tx: BaseTransaction): Boolean = tx is BaseTransactionWithRequestedAuthorization