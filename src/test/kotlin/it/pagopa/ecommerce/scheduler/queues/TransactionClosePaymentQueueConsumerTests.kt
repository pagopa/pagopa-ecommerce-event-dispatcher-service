...
class TransactionClosePaymentQueueConsumerTests {
  ...
  fun `consumer processes bare close message correctly with OK closure outcome`() = runTest {
    ...
    val expectedUpdatedTransactionClosed =
      transactionDocument(
        TransactionStatusDto.CLOSED, ZonedDateTime.parse(activationEvent.creationDate))

    val expectedUpdatedTransactionClosureError =
      transactionDocument(
    ...
    Mockito.mockStatic(UUID::class.java).use { uuid ->
      ...
      // mocking
      verify(transactionsViewRepository, Mockito.times(1)).save(expectedUpdatedTransactionClosed)
