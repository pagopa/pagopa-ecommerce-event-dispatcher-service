...
class ClosePaymentHelperTests {
  ...
  fun `consumer throw exception when the transaction authorization is performed via pgs`() =
    runTest {
      ...
      verify(transactionClosedEventRepository, Mockito.times(0))
        .insert(any<TransactionClosedEvent>())
  ...
  fun `consumer throw exception when the transaction is authorized via pgs `() = runTest {
    ...
    verify(checkpointer, Mockito.times(1)).success()
    verify(nodeService, Mockito.times(0)).closePayment(any(), any())
    verify(transactionClosedEventRepository, Mockito.times(0)).insert(any<TransactionClosedEvent>())
  ...
  fun `consumer processes bare closure error message correctly with OK closure outcome for authorization completed transaction`() =
    runTest {
      ...
      given(transactionClosedEventRepository.insert(closedEventStoreRepositoryCaptor.capture()))
      ...
      verify(transactionClosedEventRepository, Mockito.times(1))
        .insert(
          any<TransactionClosedEvent>()) // FIXME: Unable to use better argument captor because of
      // misbehaviour in static
      // mocking
  ...
  fun `consumer processes bare closure error message correctly with KO closure outcome for unauthorized transaction`() =
    runTest {
      ...
      verify(transactionClosedEventRepository, Mockito.times(1))
        .insert(
          any<
            TransactionClosureFailedEvent>()) // FIXME: Unable to use better argument captor because
      // of misbehaviour in static
      // mocking
      verify(transactionsViewRepository, Mockito.times(1)).save(expectedUpdatedTransaction)
      verify(reactivePaymentRequestInfoRedisTemplateWrapper, Mockito.after(1000).never())
        .deleteById(any())
  ...
  fun `consumer error processing bare closure error message for authorized transaction with missing authorizationResultDto value`() =
    runTest {
      ...
      given(transactionClosedEventRepository.insert(any<TransactionClosureFailedEvent>()))
  ...
  fun `consumer processes closure retry message correctly`() = runTest {
    ...
    given(transactionClosedEventRepository.insert(any<TransactionClosedEvent>()))
      .willReturn(Mono.just(expectedClosureEvent))
    given(nodeService.closePayment(TransactionId(TRANSACTION_ID), ClosePaymentOutcome.OK))
      .willReturn(
        ClosePaymentResponseDto().apply { outcome = ClosePaymentResponseDto.OutcomeEnum.OK })
  ...
  fun `consumer perform refund for authorized transaction and close payment response outcome KO for transaction in closure error status`() =
    runTest {
      ...
      verify(transactionsRefundedEventStoreRepository, Mockito.times(1))
        .insert(capture(refundedEventStoreRepositoryCaptor))
  ...
  fun `consumer enqueue retry event in case of error processing the received event for transaction in closure error status`() =
    runTest {
      ...
      given(transactionClosedEventRepository.insert(closedEventStoreRepositoryCaptor.capture()))
        .willAnswer { Mono.just(it.arguments[0]) }
      given(
          transactionClosureErrorEventStoreRepository.insert(
      ...
      verify(nodeService, Mockito.times(1))
        .closePayment(TransactionId(TRANSACTION_ID), ClosePaymentOutcome.OK)
      verify(transactionClosureErrorEventStoreRepository, Mockito.times(1))
        .insert(any<TransactionClosureErrorEvent>())
      verify(transactionClosedEventRepository, Mockito.times(1))
        .insert(any<TransactionClosedEvent>())
      verify(reactivePaymentRequestInfoRedisTemplateWrapper, Mockito.after(1000).never())
        .deleteById(any())
      verify(transactionsRefundedEventStoreRepository, Mockito.times(0))
        .save(capture(refundedEventStoreRepositoryCaptor))
      verify(transactionsViewRepository, Mockito.times(2)).save(any())
  ...
  fun `consumer enqueue retry event in case of error processing the input retry event`() = runTest {
        ...
        listOf(
          ...
          Mono.just(
            transactionDocument(
              TransactionStatusDto.CLOSURE_ERROR,
              ZonedDateTime.parse(activationEvent.creationDate)))))
    given(transactionClosureErrorEventStoreRepository.insert(any<TransactionClosureErrorEvent>()))
      .willAnswer { Mono.just(it.arguments[0]) }
    given(transactionsViewRepository.save(viewArgumentCaptor.capture()))
      .willReturnConsecutively(
        listOf(
    ...
    verify(nodeService, Mockito.times(1))
      .closePayment(TransactionId(TRANSACTION_ID), ClosePaymentOutcome.OK)
    verify(transactionClosedEventRepository, Mockito.times(1)).insert(any<TransactionClosedEvent>())
    verify(reactivePaymentRequestInfoRedisTemplateWrapper, Mockito.after(1000).never())
      .deleteById(any())
    verify(transactionsRefundedEventStoreRepository, Mockito.times(0))
      .insert(capture(refundedEventStoreRepositoryCaptor))
  ...
  fun `consumer processes closure error message correctly with KO closure outcome for unauthorized transaction with REDIRECT gateway`() =
    runTest {
      ...
      verify(reactivePaymentRequestInfoRedisTemplateWrapper, Mockito.after(1000).never())
        .deleteById(any())
      verify(transactionClosedEventRepository, Mockito.times(1))
        .insert(any<TransactionClosureFailedEvent>())
      verify(transactionsViewRepository, Mockito.times(1)).save(expectedUpdatedTransaction)
      verify(closureRetryService, times(0))
        .enqueueRetryEvent(any(), any(), any(), anyOrNull(), anyOrNull())
      assertEquals(TransactionStatusDto.UNAUTHORIZED, viewArgumentCaptor.value.status)
  ...
  fun `consumer should skip retry and refund for Nodo close payment response 400 with specific description`(
    nodoErrorDescription: String
  ) = runTest {
    whenever(mockedEnv.getProperty(ENV_TRANSACTIONS_VIEW_UPDATED_ENABLED_FLAG, "true"))
      .thenReturn("true")
    val activationEvent = transactionActivateEvent() as TransactionEvent<Any>
    ...
    given(nodeService.closePayment(TransactionId(TRANSACTION_ID), ClosePaymentOutcome.OK))
      .willThrow(
        ClosePaymentErrorResponseException(
          ...
          errorResponse = ErrorDto().outcome("KO").description(nodoErrorDescription)))
    given(
        refundQueueAsyncClient.sendMessageWithResponse(
          any<QueueEvent<TransactionRefundRequestedEvent>>(), any(), any()))
      .willReturn(queueSuccessfulResponse())
    ...
    verify(updateTransactionStatusTracerUtils, times(1))
      .traceStatusUpdateOperation(
        ClosePaymentNodoStatusUpdate(
          ...
          UpdateTransactionStatusTracerUtils.GatewayOutcomeResult(
            ClosePaymentOutcome.KO.toString(),
            Optional.of("HTTP code:[400] - descr:[$nodoErrorDescription]"))))
    val expectedViewUpdateStatuses =
      listOf(
        TransactionStatusDto.CLOSURE_ERROR,
      )
  ...
  fun `consumer skip event processing for transaction which close payment have already been processed for authorized transactions`(
    ...
    val events =
      listOf(
        ...
        closureRequestedEvent,
        closedEvent)
    val transaction = reduceEvents(*events.toTypedArray())
    /* preconditions */
    given(checkpointer.success()).willReturn(Mono.empty())
    given(
        transactionsEventStoreRepository.findByTransactionIdOrderByCreationDateAsc(TRANSACTION_ID))
    ...
    /* Asserts */
    assertEquals(TransactionStatusDto.CLOSED, transaction.status)
    verify(checkpointer, Mockito.times(1)).success()
  ...
  @ParameterizedTest
  @EnumSource(TransactionClosureData.Outcome::class)
  fun `consumer skip event processing for transaction which close payment have already been processed for unauthorized transactions`(
    closePaymentOutcome: TransactionClosureData.Outcome
  ) = runTest {
    whenever(mockedEnv.getProperty(ENV_TRANSACTIONS_VIEW_UPDATED_ENABLED_FLAG, "true"))
      .thenReturn("true")
    ...
    val authorizationCompleteEvent =
      transactionAuthorizationCompletedEvent(
        NpgTransactionGatewayAuthorizationData(
          OperationResultDto.CANCELED, "operationId", "paymentEnd2EndId", null, null))
        as TransactionEvent<Any>
    val closureRequestedEvent = transactionClosureRequestedEvent() as TransactionEvent<Any>
    val closedEvent = transactionClosureFailedEvent(closePaymentOutcome) as TransactionEvent<Any>
    val events =
      listOf(
        activationEvent,
        authorizationRequestEvent,
    ...
    /* Asserts */
    assertEquals(TransactionStatusDto.UNAUTHORIZED, transaction.status)
    verify(checkpointer, Mockito.times(1)).success()
    verify(nodeService, never()).closePayment(any(), any())
    verify(refundQueueAsyncClient, never())
      .sendMessageWithResponse(any<QueueEvent<TransactionRefundRequestedEvent>>(), any(), any())
  ...
  @ParameterizedTest
  @EnumSource(TransactionClosureData.Outcome::class)
  fun `consumer skip event processing for transaction which close payment have already been processed for user canceled transactions`(
    closePaymentOutcome: TransactionClosureData.Outcome
  ) = runTest {
    whenever(mockedEnv.getProperty(ENV_TRANSACTIONS_VIEW_UPDATED_ENABLED_FLAG, "true"))
      .thenReturn("true")
    ...
    val events = listOf(activationEvent, userCanceledEvent, closedEvent)
    ...
    StepVerifier.create(
        closePaymentHelper.closePayment(
          ClosePaymentEvent.canceled(
            QueueEvent(userCanceledEvent as TransactionUserCanceledEvent, MOCK_TRACING_INFO)),
          checkpointer,
          EmptyTransaction()))
      .expectNext(Unit)
      .verifyComplete()
    ...
    /* Asserts */
    assertEquals(TransactionStatusDto.CANCELED, transaction.status)
    verify(checkpointer, Mockito.times(1)).success()
    verify(nodeService, never()).closePayment(any(), any())
    verify(refundQueueAsyncClient, never())
      .sendMessageWithResponse(any<QueueEvent<TransactionRefundRequestedEvent>>(), any(), any())
