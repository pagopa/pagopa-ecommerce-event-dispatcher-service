...
class TransactionClosePaymentRetryQueueConsumerTests {
    ...
    runTest {
      ...
      StepVerifier.create(
          transactionClosureErrorEventsConsumer.messageReceiver(
            objectMapper
              .writeValueAsString(QueueEvent(closureErrorEvent, MOCK_TRACING_INFO))
              .encodeToByteArray(),
            checkpointer))
        .expectNext()
        .verifyComplete()
  ...
  fun `consumer processes closure retry message correctly`() = runTest {
    ...
    StepVerifier.create(
        transactionClosureErrorEventsConsumer.messageReceiver(
          objectMapper
            .writeValueAsString(QueueEvent(closureRetriedEvent, MOCK_TRACING_INFO))
            .encodeToByteArray(),
          checkpointer))
      .expectNext()
      .verifyComplete()
    ...
    verify(deadLetterQueueAsyncClient, times(1))
      .sendMessageWithResponse(
        argThat<BinaryData> {
          objectMapper
            .readValue(
              this.toBytes(), object : TypeReference<QueueEvent<TransactionClosureErrorEvent>>() {})
            .event
            .eventCode == TransactionEventCode.TRANSACTION_CLOSURE_ERROR_EVENT
        },
        eq(Duration.ZERO),
