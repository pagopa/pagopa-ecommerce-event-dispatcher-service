...
class TransactionClosePaymentRetryQueueConsumerTests {
  ...
  @Test
  fun `consumer processes bare legacy closure error message correctly with OK closure outcome for authorization completed transaction with REDIRECT gateway`() =
    runTest {
      val activationEvent = transactionActivateEvent() as TransactionEvent<Any>
      val authorizationRequestEvent =
  ...
  @Test
  fun `consumer processes bare legacy closure error message correctly with KO closure outcome for unauthorized transaction with REDIRECT gateway`() =
    runTest {
      val activationEvent = transactionActivateEvent() as TransactionEvent<Any>
      val authorizationRequestEvent =
        transactionAuthorizationRequestedEvent(
