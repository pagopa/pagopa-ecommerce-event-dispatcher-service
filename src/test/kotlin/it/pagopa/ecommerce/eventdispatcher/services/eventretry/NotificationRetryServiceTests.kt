...
class NotificationRetryServiceTests {
    ...
    fun `Should enqueue new notification retry event for left remaining attempts`() {
        ...
        StepVerifier.create(notificationRetryService.enqueueRetryEvent(baseTransaction, maxAttempts - 1))
            .expectNext()
            .verifyComplete()
        ...
        val eventSentOnQueue = queueCaptor.value.toObject(TransactionUserReceiptAddRetriedEvent::class.java)
        assertEquals(TransactionEventCode.TRANSACTION_ADD_USER_RECEIPT_RETRY_EVENT, savedEvent.eventCode)
