...
class NotificationRetryServiceTests {
  ...
  @Test
  fun `Should enqueue new notification retry event for left remaining attempts with no transactions-view update if ff disabled`() {
    whenever(mockedEnv.getProperty(ENV_TRANSACTIONS_VIEW_UPDATED_ENABLED_FLAG, "true"))
      .thenReturn("false")
    val events: MutableList<TransactionEvent<Any>> =
      mutableListOf(
  ...
  @Test
  fun `Should enqueue new notification retry event for first time sending event with no transactions-view update if ff disabled`() {
    whenever(mockedEnv.getProperty(ENV_TRANSACTIONS_VIEW_UPDATED_ENABLED_FLAG, "true"))
      .thenReturn("false")
    val events: MutableList<TransactionEvent<Any>> =
      mutableListOf(
